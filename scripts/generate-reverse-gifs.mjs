...
import { AVATAR_TRANSITION_FPS } from "./avatar-pipeline-config.mjs";
...
const TARGET_FPS = AVATAR_TRANSITION_FPS;
...
function reverseGif(inputPath, outputPath) {
  const result = spawnSync(
    ...
    [
      ...
      inputPath,
      "-filter_complex",
      `[0:v] fps=${TARGET_FPS},reverse,split [rev_palette][rev_out];[rev_palette] palettegen [palette];[rev_out][palette] paletteuse`,
      "-gifflags",
      "-offsetting",
      "-loop",
      "0",
...
function main() {
  ...
  if (!ffmpeg.available) {
    ...
    for (const file of files) {
      ...
      const result = reverseGif(inputPath, outputPath);
  ...
  console.log(`Target playback rate: ${TARGET_FPS} fps.`);
